...
sys.stdout = open('pointerWithCov.log','w')
...
def outputVar(l1, l2, voc):
    indexes_batch = [indexesFromSentence(voc, sentence) for sentence in l2]
    ...
    return padVar, mask, max_target_len, pointerVar
...
def batch2TrainData(voc, pair_batch):
    ...
    output, mask, max_target_len, pointers = outputVar(input_batch, output_batch, voc)
    return inp, lengths, output, mask, max_target_len, pointers


# Example for validation
small_batch_size = 5
batches = batch2TrainData(voc, [random.choice(pairs) for _ in range(small_batch_size)])
input_variable, lengths, target_variable, mask, max_target_len, pointers = batches
...
def trainIters(model_name, voc, pairs, encoder, decoder, encoder_optimizer, decoder_optimizer, embedding,
    ...
    for iteration in range(start_iteration, n_iteration + 1):
        ...
        input_variable, lengths, target_variable, mask, max_target_len, pointers = training_batch

        coverage = torch.zeros(batch_size, max(lengths))

        # Run a training iteration with batch
        loss = train(input_variable, lengths, pointers, mask, max_target_len, encoder,
